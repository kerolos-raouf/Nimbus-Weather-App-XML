...
class DetailsViewModel @Inject constructor(
    ...
    private fun fillDaysWeather(list : List<WeatherItemEveryThreeHours>)
    {
        ...
        val dayFourIndex = 25
        val dayFiveIndex = 33
        _dayOne.value = DaysWeather(
            convertUnixToDay(list[dayOneIndex].dt.toLong(),format),
            capitalizeWord(list[dayOneIndex].weather[0].description),list[dayOneIndex].main.tempMin.toString(),list[dayTwoIndex].main.tempMax.toString())
        _dayTwo.value = DaysWeather(
            convertUnixToDay(list[dayTwoIndex].dt.toLong(),format),
            capitalizeWord(list[dayTwoIndex].weather[0].description),list[dayTwoIndex].main.tempMin.toString(),list[dayTwoIndex].main.tempMax.toString())
        _dayThree.value = DaysWeather(
            convertUnixToDay(list[dayThreeIndex].dt.toLong(),format),
            capitalizeWord(list[dayThreeIndex].weather[0].description),list[dayThreeIndex].main.tempMin.toString(),list[dayTwoIndex].main.tempMax.toString())
        _dayFour.value = DaysWeather(
            convertUnixToDay(list[dayFourIndex].dt.toLong(),format),
            capitalizeWord(list[dayFourIndex].weather[0].description),list[dayFourIndex].main.tempMin.toString(),list[dayTwoIndex].main.tempMax.toString())
